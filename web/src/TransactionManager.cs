...
namespace SafePuchaseWeb
{
    ...
    // TODO: remove this copy of TransactionManager once official version has magic support
    public class TransactionManager
    {
        ...
        public TransactionManager(RpcClient rpc, uint magic)
        {
            rpcClient = rpc;
        ...
        private long CalculateNetworkFee()
        {
            ...
            int size = Transaction.HeaderSize + Tx.Signers.GetVarSize() + Tx.Attributes.GetVarSize() + Tx.Script.GetVarSize() + Neo.IO.Helper.GetVarSize(hashes.Length);
        ...
        public TransactionManager Sign()
        {
            ...
            foreach (var item in signStore)
                ...
                {
                    byte[] signature = Sign(Tx, key, magic);
            ...
            static byte[] Sign(IVerifiable verifiable, KeyPair key, uint magic)
            {
                return Neo.Cryptography.Crypto.Sign(GetHashData(verifiable, magic), key.PrivateKey, key.PublicKey.EncodePoint(false)[1..]);
            }
        }
    }
    #nullable restore
